...
filteredDataSelected <- filteredData %>%
  select(-PurchDate, -Cylinder, -WheelDrive, -Liter, -Doors)
...
```{r}
library(tuneRanger)
modelData.task = makeClassifTask(data = modelData, target = "IsBadBuy")
estimateTimeTuneRanger(modelData.task)

res = tuneRanger(modelData.task,
                 measure = list(auc),
                 tune.parameters = c("mtry", "min.node.size", "max.depth"),
                 num.trees = 1000
                 )
```
...
```{r}
rf_01 <- ranger(IsBadBuy ~ ., data = data_training, importance = "impurity", probability = TRUE, seed=42, num.trees = 750)
print(rf_01)
importance(rf_01)
...
predictions_validation_binary_01 <- ifelse(predictions_validation_01>0.6, "1" ,"0")
