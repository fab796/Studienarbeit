...
filteredDataBalanced <- ovun.sample(IsBadBuy ~ ., data = filteredData, method = "under", p = 0.2)$data
...
FÜhre Rander Tuning aus, um passendes Model zu erhalten
```{r}
library(tuneRanger)
...
shinyApp(
  ...
  ui = fluidPage(
    ...
    fluidRow(
      ...
      column(8,
             h4("Dateneingabe:"),
             column(6,
                    selectInput("Auction", "Auction:", choices = c(unique(training$Auction))),
                textInput("Make", "Make:"),    
                textInput("Model", "Model:"),
                textInput("SubModel", "SubModel:"),
                textInput("Color", "Color:"),
                textInput("Trim", "Trim:"),
                textInput("WheelType", "WheelType:"),
                textInput("Nationality", "Nationality:"),
                textInput("Size", "Size:"),
                textInput("TopThreeAmericanName", "TopThreeAmericanName:"),
                textInput("VNST", "VNST:"),
                dateInput("PurchDate", "PurchDate:"),
                textInput("Cylinder", "Cylinder:")),
            column(6,
                textInput("Transmission", "Transmission:"),
                ...
                numericInput("VehYear", "VehYear:", value = 0),
  ...
  server = function(input, output) {
    ...
    myModel <- readRDS("trainedModel.rds")
    ...
    observeEvent(input$Berechnen, {
      ...
      output$result <- renderDataTable(df)
        ...
        predictions_validation_binary_01 <- ifelse(predictions_validation_01>0.5,"1","0")
        data_validation_w_preds_01 <- cbind(df, predictions_validation_01, predictions_validation_binary_01)
      
      output$pred <- renderText({
        paste("Dein Fahrzeug ist zu",
          data_validation_w_preds_01$predictions_validation_01 * 100, "Prozent ein Fehlkauf..", sep = " ")
        })
    })
